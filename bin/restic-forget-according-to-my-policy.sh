...
# This 'forget' operation makes sure to keep 14 daily, 6 weekly, 18 monthly and 8 yearly backups.
${restic} -r "${repo}" -p "${pwfile}" --verbose=4 \
    forget --host "${host}" --keep-daily 14 --keep-weekly 6 --keep-monthly 18 --keep-yearly 8 2>&1 \
